...
if($_GET['acao'] == 'alterar') {
  ...
  $texto = addslashes(trim(fonte_editor($_POST['texto'])));
	...
	if($altera) {
		// Log
		salvar_log($_SESSION["id_usuario"],"alterar area",$sql_alterar, $pdo);
	
		echo "<script>";
		echo "open('gerenciar_diferenciais.php?aviso=ok&msg=Informações alteradas com sucesso.','_self');";
		echo "</script>";
	} else {
		// Log
		...
		echo "<script>";
		echo "open('".$_SERVER['SCRIPT_NAME']."?aviso=erro&msg=Não foi possível alterar as informações.&id_diferencial=".base64_encode($id_diferencial)."','_self');";
		echo "</script>";
	}
}
?>
...
<div class="titulo-sessao-azul">Alterar Área de Atuação</div>
