...
if($_GET['acao'] == 'incluir') {
  ...
  $query_inclui->execute();
	...
	if($query_inclui) {
		...
		echo "<script>";
		echo "open('gerenciar_diferenciais.php?aviso=ok&msg=Área incluída com sucesso.','_self');";
		echo "</script>";
	} else {
		// Log
		...
		echo "<script>";
		echo "open('".$_SERVER['SCRIPT_NAME']."?aviso=erro&msg=Não foi possível incluir a área de atuação.','_self');";
		echo "</script>";
	}
}
?>
...
<div class="titulo-sessao-azul">Incluir Área de Atuação</div>
