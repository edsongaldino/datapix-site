...
if($_GET['acao'] == 'incluir') {
  ...
  $funcao = addslashes(trim($_POST['funcao']));
	...
	// Inclui o produto
	$sql_inclui = "INSERT INTO equipe (titulo_equipe,texto_equipe,funcao,arquivo,status) VALUES ('".$titulo."','".$texto."', '".$funcao."','".$nome_arquivo."','".$status."')";
  ...
  $query_inclui->execute();
	...
	if($query_inclui) {
		// Log
		salvar_log($_SESSION["id_usuario"],"incluir equipe",$sql_inclui,$pdo);

		echo "<script>";
		echo "open('gerenciar_equipe.php?aviso=ok&msg=Membro incluído com sucesso.','_self');";
		echo "</script>";
	} else {
		// Log
		...
		echo "<script>";
		echo "open('".$_SERVER['SCRIPT_NAME']."?aviso=erro&msg=Não foi possível incluir o item.','_self');";
		echo "</script>";
	}
}
?>
...
<div class="titulo-sessao-azul">Incluir membro na equipe</div>
...
<form id="form1" name="form1" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?acao=incluir" enctype="multipart/form-data" onsubmit="javascript: this.button.value='Aguarde, carregando...'; this.button.disabled='disabled';">
          ...
          <div class="TabbedPanelsContent">
            <table width="100%" border="0" cellspacing="3" cellpadding="0">
              <tr>
                ...
                <td><input name="titulo_equipe" type="text" id="titulo_equipe" size="50" maxlength="150" /></td>
              </tr>
              <tr>
                <td class="campo-form">Função:</td>
                <td><input name="funcao" type="text" id="funcao" size="50" maxlength="150" /></td>
              </tr>
              <tr>
  ...
  <tr>
    ...
    <td><div align="right"><input type="submit" name="button" id="button" onclick="" value="Salvar novo equipe" /></div></td>
  </tr>
</form>
</table>
