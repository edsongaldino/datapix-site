...
// Pega os dados
$sql_equipe = "SELECT titulo_equipe, funcao, texto_equipe, arquivo, status FROM equipe WHERE id_equipe = '".$id_equipe."'";
$query_equipe = $pdo->query($sql_equipe);
$executa_equipe = $query_equipe->fetch( PDO::FETCH_ASSOC );

if($_GET['acao'] == 'alterar') {
	// PEGA OS DADOS DO FORMULÁRIO
  $titulo = addslashes(trim($_POST['titulo_equipe']));
  $funcao = addslashes(trim($_POST['funcao']));
    ...
    $nome_arquivo = $executa_equipe["arquivo"];
		
	} else {
   
    $uploaddir = '../../conteudos/equipe/';
	...
	try{
		...
		// REALIZA A ALTERAÇÃO
		$sql_alterar = "UPDATE equipe SET titulo_equipe = '".$titulo."', texto_equipe = '".$texto."', funcao = '".$funcao."', arquivo = '".$nome_arquivo."', status = '".$status."' WHERE id_equipe = ".$id_equipe."";
		$atualiza_equipe = $pdo->prepare($sql_alterar);
		$atualiza_equipe->execute();
	...
	if($altera) {
		// Log
		salvar_log($_SESSION["id_usuario"],"alterar equipe",$sql_alterar, $pdo);
	
		echo "<script>";
		echo "open('gerenciar_equipe.php?aviso=ok&msg=Informações da equipe alterada com sucesso.','_self');";
		echo "</script>";
	} else {
		// Log
		...
		echo "<script>";
		echo "open('".$_SERVER['SCRIPT_NAME']."?aviso=erro&msg=Não foi possível alterar as informações do equipe.&id_equipe=".base64_encode($id_equipe)."','_self');";
		echo "</script>";
	}
}
?>
...
<body onload="document.form1.titulo_equipe.focus();">
<div class="titulo-sessao-azul">Alterar equipe</div>
<?php include("../sistema_aviso.php"); ?>
<table width="90%" border="0" align="center" cellpadding="0" cellspacing="0">
<form id="form1" name="form1" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?acao=alterar&id_equipe=<?php echo base64_encode($id_equipe); ?>"  enctype="multipart/form-data" onsubmit="javascript: this.button.value='Aguarde, carregando...'; this.button.disabled='disabled';">
  <tr>
    <td colspan="2">
      <div id="alterar_equipe" class="TabbedPanels">
          ...
          <div class="TabbedPanelsContent">
            <table width="100%" border="0" cellspacing="3" cellpadding="0">
              <tr>
                ...
                <td><input name="titulo_equipe" type="text" id="titulo_equipe" value="<?php echo $executa_equipe['titulo_equipe']; ?>" size="50" maxlength="150" /></td>
              </tr>
              <tr>
                <td class="campo-form">Função:</td>
                <td><input name="funcao" type="text" id="funcao" value="<?php echo $executa_equipe['funcao']; ?>" size="50" maxlength="150" /></td>
              </tr>
              <tr>
                ...
                <td>
                    ...
                    <textarea name="texto" cols="60" rows="10" id="texto"><?php echo $executa_equipe['texto_equipe']; ?></textarea>
              ...
              <tr>
                ...
                <select name="status" id="status">
                  <option value="L" <?php if($executa_equipe['status'] == "L") { echo 'selected="selected"'; } ?>>Liberado</option>
                  <option value="B" <?php if($executa_equipe['status'] == "B") { echo 'selected="selected"'; } ?>>Bloqueado</option>
                </select>
                </td>
              </tr>
            </table>
...
var alterar_equipe = new Spry.Widget.TabbedPanels("alterar_equipe");
</script>
</body>
</html>