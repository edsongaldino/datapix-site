...
if($_GET['acao'] == 'alterar') {
	...
	if($altera) {
		...
		echo "<script>";
		echo "open('gerenciar_diferenciais.php?aviso=ok&msg=Informações da diferencial alterada com sucesso.','_self');";
		echo "</script>";
	} else {
		// Log
		...
		echo "<script>";
		echo "open('".$_SERVER['SCRIPT_NAME']."?aviso=erro&msg=Não foi possível alterar as informações do diferencial.&id_diferencial=".base64_encode($id_diferencial)."','_self');";
		echo "</script>";
	}
}
?>
...
<div class="titulo-sessao-azul">Alterar diferencial</div>
