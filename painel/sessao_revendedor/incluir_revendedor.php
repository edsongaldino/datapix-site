...
if($_GET['acao'] == 'incluir') {
	...
	$titulo = addslashes(trim($_POST['nome_revendedor']));
	...
	if($arquivo) {
		...
		$uploaddir = '../../conteudos/revendedor/';
	...
	// Inclui o produto
	$sql_inclui = "INSERT INTO revendedor (nome_revendedor,texto_revendedor,arquivo,status) VALUES ('".$titulo."','".$texto."','".$nome_arquivo."','".$status."')";
  ...
  $query_inclui->execute();
	...
	if($query_inclui) {
		...
		echo "<script>";
		echo "open('gerenciar_diferenciais.php?aviso=ok&msg=revendedor incluído com sucesso.','_self');";
		echo "</script>";
	} else {
		// Log
...
// Busca estados
$sql_estados = "SELECT codigo_estado, nome_estado FROM estado ORDER BY nome_estado ASC";
