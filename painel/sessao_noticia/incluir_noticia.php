...
if($_GET['acao'] == 'incluir') {
  ...
  $data = addslashes(trim($_POST['data']));
	...
	// Inclui o produto
	$sql_inclui = "INSERT INTO noticia (titulo_noticia,texto_noticia,resumo_noticia,data_noticia,arquivo,status) VALUES ('".$titulo."','".$texto."', '".$resumo."','".$data."', '".$nome_arquivo."','".$status."')";
  ...
  $query_inclui->execute();
	...
	if($query_inclui) {
		// Log
		salvar_log($_SESSION["id_usuario"],"incluir noticia",$sql_inclui,$pdo);

		echo "<script>";
		echo "open('gerenciar_noticias.php?aviso=ok&msg=noticia incluído com sucesso.','_self');";
		echo "</script>";
	} else {
		// Log
		...
		echo "<script>";
		echo "open('".$_SERVER['SCRIPT_NAME']."?aviso=erro&msg=Não foi possível incluir o noticia.','_self');";
		echo "</script>";
	}
}
?>
...
<div class="titulo-sessao-azul">Incluir nova noticia</div>
...
<form id="form1" name="form1" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?acao=incluir" enctype="multipart/form-data" onsubmit="javascript: this.button.value='Aguarde, carregando...'; this.button.disabled='disabled';">
          ...
          <div class="TabbedPanelsContent">
            <table width="100%" border="0" cellspacing="3" cellpadding="0">
              <tr>
                ...
                <td><input name="titulo_noticia" class="campo-nome" type="text" id="titulo_noticia" size="50" maxlength="150" /></td>
              </tr>
              <tr>
                <td class="campo-form">Imagem:</td>
                <td><input name="arquivo" type="file" class="campo-nome" id="arquivo" size="50" /> (.png *)</td>
              </tr>

              <tr>
                <td valign="top" class="campo-form">Resumo:</td>
                <td><textarea name="resumo" cols="60" rows="3" id="resumo" maxlength="200"></textarea></td>
              </tr>
              <tr>
                <td class="campo-form">Data:</td>
                <td><input name="data" type="date" id="data_noticia" class="campo-data" size="50" /></td>
              </tr>
							<tr>
  ...
  <tr>
    ...
    <td><div align="right"><input type="submit" name="button" id="button" onclick="" value="Salvar novo noticia" /></div></td>
  </tr>
</form>
</table>
