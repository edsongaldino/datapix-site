...
if($_GET['acao'] == 'incluir') {
  ...
  $localizacao = addslashes(trim($_POST['localizacao']));
	
	// Inclui o galeria
	$sql_inclui = "INSERT INTO galerias (nome,descricao,texto,localizacao,status) VALUES ('".$nome."','".$descricao."','".$texto."','".$localizacao."','".$status."')";
  ...
  $query_inclui->execute();
    ...
    $ftp_chmod_pasta_temp = chmod("../../conteudos/galeria/".$id_galeria."/temp", 0777);
		...
		if(($ftp_cria_pasta_raiz) && ($ftp_cria_pasta_mini) && ($ftp_cria_pasta_destaque) && ($ftp_cria_pasta_temp) && ($ftp_chmod_pasta_temp)) {
			...
			echo "<script>";
			echo "open('incluir_galeria_fotos_destaque.php?id_galeria=".base64_encode($id_galeria)."&aviso=ok&msg=Galeria incluída com sucesso.','_self');";
			echo "</script>";
		} else {
			// Log
...
<div class="titulo-sessao-azul">Incluir Fazenda</div>
...
<form id="form1" name="form1" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?acao=incluir" enctype="multipart/form-data" onsubmit="javascript: this.button.value='Aguarde, carregando...'; this.button.disabled='disabled';">
          ...
          <div class="TabbedPanelsContent">
            <table width="100%" border="0" cellspacing="3" cellpadding="0">
              ...
              <tr>
                ...
                <td><input name="descricao" type="text" id="descricao" size="50" maxlength="150" placeholder="Resumo" /></td>
              </tr>
              <tr>
                <td class="campo-form">Localização:</td>
                <td><input name="localizacao" type="text" id="localizacao" size="50" maxlength="150" placeholder="Cidade, UF" /></td>
              </tr>
              <tr>
  ...
  <tr>
    ...
    <td><div align="right"><input type="submit" name="button" id="button" value="Salvar nova galeria" /></div></td>
  </tr>
</form>
</table>
